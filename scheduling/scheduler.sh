...
python3 scheduling/scripts/extraction_graphe.py "$IR_file" > "${base}_graphe.json"

# visualisation du graphe de flot et de dépendance avec graphviz
python3 scheduling/scripts/visu_graphe.py "${base}_graphe.json"

#list scheduling des opérations présente dans le json de dépendance/flot
python3 scheduling/scripts/list_scheduling.py "${base}_graphe.json"

# deduction du nom du json de scheduling a donné en paramètre à  l'outil de reda
echo "${base}_graphe_schedule.json"